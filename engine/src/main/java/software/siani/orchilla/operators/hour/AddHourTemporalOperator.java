...
public record AddHourTemporalOperator(int value) implements TemporalOperator {

    @Override
    public TemporalTag computeFor(TemporalTag temporaltag) {
        if (temporaltag.period().compareTo(Period.Hour) <= 0) {
            LocalDateTime start = temporaltag.head().plusHours(value);
            LocalDateTime end = temporaltag.tail().plusHours(value);
            return new TemporalTag(start, end, temporaltag.period(), temporaltag.distribution().between(start, end));
        }
        LocalDateTime start = temporaltag.head().plusHours(value);
        LocalDateTime end = temporaltag.tail().plusHours(value).plusHours(1).minusNanos(1);
