...
var launchApplication = function () {
	...
	function renderApplication() {
		...
		const content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><DocTemplatePage/></GoogleApiLoader> : <DocTemplatePage/>;
		const docTemplate = document.getElementById("DocTemplate");
		if (docTemplate) ReactDOM.render(<DisplayRouter id="__router__" owner={()=>""} context={()=>""}>{content}</DisplayRouter>, docTemplate);
...
function loadAppUrl() {
	...
	url = url.replace(new RegExp("\\/docs", 'g'), "");
