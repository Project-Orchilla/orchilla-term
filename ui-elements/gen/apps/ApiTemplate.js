...
import ApiTemplatePage from "../pages/ApiTemplatePage";
...
var launchApplication = function () {
	...
	function renderApplication() {
		...
		const content = requireGoogleApi ? <GoogleApiLoader onLoad={openPushService()}><ApiTemplatePage/></GoogleApiLoader> : <ApiTemplatePage/>;
		const apiTemplate = document.getElementById("ApiTemplate");
		if (apiTemplate) ReactDOM.render(<DisplayRouter id="__router__" owner={()=>""} context={()=>""}>{content}</DisplayRouter>, apiTemplate);
