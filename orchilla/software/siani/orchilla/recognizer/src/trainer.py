...
class RecognizerTrainer:
	...
	def train(self, dataset: list, path: str, language: str = "en"):
		...
		train(f"../res/recognizer.cfg", "./output",
			  overrides={"paths.train": "./train.spacy", "paths.dev": "./train.spacy"})
	...
	def __prepare_dataset(self, language: str, data: list):
		...
		for source, target in data:
			...
			for start, end, label in target:
				if (
				span := doc.char_span(int(start), int(end), label=label, alignment_mode="contract")) is None: continue
				ents.append(span)
			doc.ents = ents
			dataset.add(doc)
	...
	def __save_model(self, path: str, language: str):
		...
		with open(f"{path}/{language}/recognizer.mdl", mode='wb') as f:
...
if __name__ == "__main__":
	RecognizerTrainer().train(DetectorDatasetReader.read("data/en/train.tsv"), "model", "en")
