...
class SetDateEntryTemplate(DateEntryTemplate):
    ...
    def __call__(self):
        ...
        if probability < 1/8: return self.__generate_weekday()
        if probability < 2/8: return self.__generate_day()
        if probability < 3/8: return self.__generate_month()
        if probability < 4/8: return self.__generate_year()
        if probability < 5/8: return self.__generate_day_month()
        if probability < 6/8: return self.__generate_month_year()
        if probability < 7/8: return self.__generate_festivity()
        return self.__generate_default()

    def __generate_weekday(self):
        weekday = random.choice(self.Weekdays)
        return "set {}\t{}".format(weekday.lower(), random.choice(self.WeekdayTemplates).format(weekday))

    def __generate_day(self):
        day = self.number(32)
        return "set d{:02d}\t{}".format(day, random.choice(self.DayTemplates).format(self.format_day(day)))

    def __generate_month(self):
        month = random.choice(self.Months)
        return "set d01M{}\t{}".format(month.lower(), random.choice(self.MonthTemplates).format(month))

    def __generate_year(self):
        year = self.__randomize_year()
        return "set d01M01Y{:04d}\t{}".format(year, random.choice(self.YearTemplates).format(year))
    ...
    def __generate_day_month(self):
        ...
        return "set d{:02d}M{}\t{}".format(day, month.lower(), entry)

    def __generate_month_year(self):
        month = random.choice(self.Months)
        year = self.__randomize_year()
        return "set d01M{}Y{:04d}\t{}".format(month.lower(), year, random.choice(self.MonthYearTemplates).format(month, year))

    def __generate_festivity(self):
        festivity = random.choice(self.Festivities)
        return "set f{}\tin {}".format(festivity)
    ...
    def __generate_default(self):
        ...
        return "set d{:02d}M{}Y{:04d}\t{}".format(day, month.lower(), year, entry)
...
class SetTimeEntryTemplate(DateEntryTemplate):
    ...
    AmTemplates = ["at {:02d}:{:02d} AM", "at {:02d} {:02d} AM", "at {:02d} {:02d}AM", "at {:02d} {:02d}AM", "at {:02d} {:02d} in the morning", "at {:02d}:{:02d} in the morning"]
    PmTemplates = ["at {:02d}:{:02d} PM", "at {:02d} {:02d} PM", "at {:02d} {:02d}PM", "at {:02d} {:02d}PM", "at {:02d} {:02d} in the afternoon", "at {:02d} {:02d} in the evening", "at {:02d}:{:02d} in the afternoon", "at {:02d}:{:02d} in the evening"]
    TimeTemplates = ["at {:02d}:{:02d}", "at {:02d} {:02d}"]
    ...
    def __generate_hour(self):
        hour = self.number(24)
        return "set h{:02d}\t{}".format(hour, random.choice(self.HourTemplates).format(hour))
