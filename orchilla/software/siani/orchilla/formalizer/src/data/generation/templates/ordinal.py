...
class OrdinalDateEntryTemplate(DateEntryTemplate):
    ...
    def __generate_hour(self):
        ...
        return "{} h\t{}".format(ordinal, random.choice(self.HourTemplates).format(ordinal))

    def __generate_day(self):
        ordinal = random.choice(self.Ordinals)
        return "{} d\t{}".format(ordinal, random.choice(self.DayTemplates).format(ordinal))

    def __generate_weekday(self):
        ordinal = random.choice(self.Ordinals)
        weekday = random.choice(self.Weekdays)
        return "{} wd{}\t{}".format(ordinal, weekday, random.choice(self.WeekdayTemplates).format(ordinal, weekday))

    def __generate_weekend(self):
        ordinal = random.choice(self.Ordinals)
        return "{} we\t{}".format(ordinal, random.choice(self.WeekendTemplates).format(ordinal))
    ...
    def __generate_quarter(self):
        ...
        return "{} Q\t{}".format(ordinal, random.choice(self.QuarterTemplates).format(ordinal))

    def __generate_semester(self):
        ordinal = random.choice(self.Ordinals[0:2])
        return "{} SM\t{}".format(ordinal, random.choice(self.SemesterTemplates).format(ordinal))